...
const getBemTag = (tagName: string): BemTag => 
	forwardRef(
		({children, className, modifiers, ...restProps}: PropsWithChildren<BemTagProps>, ref: React.Ref<any>) => {
			...
			const modifiedBlocks = blockClassName && modifiers ? modifiers.map(modifier => `${blockClassName}--${modifier}`) : [];
			const modifiedClassName = modifiedBlocks.length ? `${className} ${modifiedBlocks.join(' ')}` : className;
